...
(|||) f s = P (\i -> optional id (parse s i) (ifFull (not . isErrorResult) (parse f i)))
...
instance Monad Parser where
  ...
  (=<<) f pa = P (\i -> parse pa i `onResult` (\i' a -> parse (f a) i'))
...
satisfy f = (\a -> P (\i -> if f a then Result i a else UnexpectedChar a)) =<< character
...
sequenceParser ::
  List (Parser a) ->
  Parser (List a)
sequenceParser lpa = foldRight (.:.) (pure Nil) =<< pure lpa
