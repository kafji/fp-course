...
findM f = foldRight (\a z -> f a >>= (\b -> if b then pure (Full a) else z)) (pure Empty)
...
isHappy n = contains 1 (firstRepeat (squaredSum (fromInteger n)))

-- | Given an integer, returns list of its sum of the square of its digits.
--
-- >>> take 10 (squaredSum 4)
-- [4,16,37,58,89,145,42,20,4,16]
--
-- >>> take 10 (squaredSum 7)
-- [7,49,97,130,10,1,1,1,1,1]
squaredSum = produce (sum . squared)

squared n = square . digitToInt <$> show' n
